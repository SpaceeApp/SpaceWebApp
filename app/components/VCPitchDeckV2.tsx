...
const SpaceLogo = ({ size = 160 }: { size?: number }) => (
  <img
    src={logoSrc}
    width={size}
    style={{ height: 'auto', filter: 'url(#remove-white)', display: 'block' }}
    alt="SPACE"
  />
);
    ...
    <div className="flex-1 flex flex-col justify-center mt-20">
      ...
      <div className="flex items-end gap-6 mb-14">
        <h1
          ...
          style={{ fontSize: '11rem', fontFamily: "'Conthrax', sans-serif" }}
        >
          SPACE
        </h1>
...
const STEP_MS = 1500;
const TOTAL_PROBLEM_STEPS = 3 + PROBLEM_MESSAGES.length;
...
const ChatScreen = ({ messageCount }: { messageCount: number }) => (
  <div className="absolute inset-0 flex flex-col animate-screen-on" style={{ backgroundColor: '#0b141a' }}>
    ...
    <div className="flex-1 flex flex-col justify-end gap-1.5 p-3 overflow-hidden"
      ...
      {PROBLEM_MESSAGES.slice(0, messageCount).map((msg, i) =>
        msg.self ? (
          <div
            key={i}
            className="bg-[#005c4b] rounded-2xl rounded-tr-sm py-2 px-3 max-w-[78%] animate-message-in shadow-sm self-end"
          >
          ...
          <div
            key={i}
            className="bg-[#1f2c33] rounded-2xl rounded-tl-sm py-2 px-3 max-w-[78%] animate-message-in shadow-sm self-start"
          >
...
const SlideProblem = ({ index, total }: SlideProps) => {
  ...
  useEffect(() => {
    if (step >= TOTAL_PROBLEM_STEPS - 1) return;
    const t = setTimeout(() => setStep(step + 1), STEP_MS);
    return () => clearTimeout(t);
  }, [step]);

  // Derive UI state from step
  ...
  if (step === 0) phoneState = 'off';
    ...
    phoneState = 'chat';
    messageCount = Math.min(step - 2, PROBLEM_MESSAGES.length);
  }
      ...
      <div className="flex-1 flex items-center justify-between gap-16 mt-20">
        ...
        <div className="shrink-0 relative">
          ...
          <PhoneFrame>
            ...
            {phoneState === 'chat' && <ChatScreen key="chat" messageCount={messageCount} />}
          </PhoneFrame>
        </div>
...
const SmallPhone = ({ children, lifted }: { children: React.ReactNode; lifted?: boolean }) => (
  <div
    ...
    style={{
      boxShadow:
        '0 0 0 6px #1a1a1e, 0 0 0 7px #2a2a2e, 0 25px 50px -12px rgba(94,92,230,0.25)',
    }}
  >
...
const PhoneStatusBar = ({ light = true }: { light?: boolean }) => (
  <div
    className={`pt-2 pb-1 px-4 flex justify-between items-center text-[9px] font-semibold relative z-10 ${
      light ? 'text-white' : 'text-black'
    }`}
  >
    <span>9:41</span>
    <span className="flex gap-1 items-center">
      <svg className="w-3 h-2.5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M5 9h2v9H5zm4-3h2v12H9zm4-3h2v15h-2zm4 6h2v9h-2z" />
      </svg>
...
const FolderRow = ({
  ...
  <div className="rounded-xl bg-white/[0.04] border border-white/10 overflow-hidden flex shrink-0">
    <div className={`w-14 h-14 bg-gradient-to-br ${cover} shrink-0`} />
    <div className="flex-1 px-2.5 py-1.5 flex flex-col justify-center min-w-0">
      <h4 className="text-white text-[12px] font-semibold tracking-tight truncate">{title}</h4>
      <p className="text-gray-500 text-[9px] mt-0.5 truncate">{sub}</p>
...
// ── Phone Screen 2: Gallery (inside a Space) ────────────────────────────────
...
// ── Phone Screen 3: Photo detail with comments ──────────────────────────────
const MemoryScreen = () => (
...
const PhoneShowcase = ({
  ...
  <div className="flex flex-col items-center gap-5 animate-rise" style={{ animationDelay: delay }}>
    ...
    <div className="text-center">
      <p className="text-[#5E5CE6] text-[10px] font-mono tracking-[0.25em] uppercase mb-1.5">
        {label}
      </p>
...
// Order: Cross-Platform, Highest Quality, Collab Folders, Zero Clutter, Social
    ...
    <div className="flex-1 flex flex-col justify-center mt-20 mb-6 max-w-6xl mx-auto w-full">
      <div className="grid grid-cols-[1.5fr_1fr_1fr_1fr_1fr_1fr] gap-x-4 gap-y-1 items-center">
        ...
        {MATRIX_COLUMNS.map((col, i) => (
          <div
            key={i}
            className="font-bold text-white text-center text-base uppercase tracking-wide border-b border-gray-800 pb-4 leading-tight"
          >
            {col.l1}
            <br />
            {col.l2}
...
const SlideMarketTrends = ({ index, total }: SlideProps) => {
  // x: 2013(38)→2025(292) step≈21 | y: scaled 0–2.5T over 83px, baseline y=95
    ...
    <div className="flex flex-col h-full px-20 relative w-full">
      ...
      <div className="flex flex-1 flex-col justify-center mt-10 max-w-5xl mx-auto w-full">
        ...
        <div className="grid grid-cols-3 gap-6 w-full">
          ...
          <div className="bg-[#1a1a1e] border border-gray-800 rounded-3xl p-7 flex flex-col">
            ...
            <div className="text-[#5E5CE6] font-black text-4xl mb-1">2.1T photos / year</div>
            <p className="text-gray-500 text-sm">taken worldwide · +223% vs. 2013</p>
          </div>

          {/* ── Card 2: Platforms ── */}
          <div className="bg-gradient-to-b from-[#5E5CE6]/20 to-[#1a1a1e] border border-[#5E5CE6]/30 rounded-3xl p-7 flex flex-col relative overflow-hidden">
            ...
            <div className="text-white font-black text-4xl mb-1">7B photos / day</div>
          ...
          <div className="bg-[#1a1a1e] border border-gray-800 rounded-3xl p-7 flex flex-col">
            ...
            <div className="text-[#5E5CE6] font-black text-4xl mb-1">3× more photos</div>
          ...
          <React.Fragment key={block.eyebrow}>
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                <span style={{ color: block.accent }}>{block.icon}</span>
                <p className="text-[10px] font-mono tracking-[0.25em] uppercase" style={{ color: block.accent }}>
                  {block.eyebrow}
                </p>
              </div>
    ...
    <div className="flex-1 flex flex-col justify-center mt-20 max-w-5xl mx-auto w-full">
      ...
      <div className="grid grid-cols-4 gap-6">
        {TEAM_MEMBERS.map((member) => (
          <div
            key={member.name}
            className="bg-[#0e0e16] border border-gray-800 rounded-3xl overflow-hidden flex flex-col"
          >
            {/* Photo area */}
            ...
            <div className="px-6 py-5 flex items-end justify-between">
              <div>
                ...
                <p className="text-[#5E5CE6] text-[10px] font-mono tracking-[0.2em] uppercase">
                  {member.role}
                </p>
              </div>
              <a
  ...
  <div className="flex flex-col items-center justify-center h-full text-center relative w-full">
    ...
    <div className="relative z-10 flex flex-col items-center gap-8">
      <h1
        ...
        style={{ fontSize: '13rem', lineHeight: '0.9', fontFamily: "'Conthrax', sans-serif" }}
      >
        SPACE
      </h1>
      ...
      <div className="flex gap-3">
        {[
          { label: 'LinkedIn',  icon: <LinkedInIcon />,  href: 'https://www.linkedin.com/company/space-app-official/' },
          { label: 'Instagram', icon: <InstagramIcon />, href: 'https://www.instagram.com/spaceeapp' },
          { label: 'TikTok',   icon: <TikTokIcon />,    href: 'https://www.tiktok.com/@spaceeapp' },
          { label: 'X',        icon: <XIcon />,         href: 'https://x.com/spaceeapp' },
        ].map((s) => (
          <button
            key={s.label}
  ...
  return (
    <>
      <style>{`
        ...
        /* Slide 03 — phone animation */
        @keyframes screen-on {
          0% { opacity: 0; }
          100% { opacity: 1; }
        }
        .animate-screen-on {
          animation: screen-on 0.5s ease-out both;
        }
        @keyframes notif-in {
        ...
        .animate-notif-in {
          animation: notif-in 0.55s cubic-bezier(0.2, 0.8, 0.2, 1) both;
        }
        ...
        .animate-message-in {
          animation: message-in 0.4s cubic-bezier(0.2, 0.8, 0.2, 1) both;
        }

        /* Slide 04 — solution stagger */
        @keyframes line-rise {
          0% { opacity: 0; transform: translateY(40px); }
          100% { opacity: 1; transform: translateY(0); }
        }
        .animate-line-1 {
          animation: line-rise 0.9s cubic-bezier(0.2, 0.8, 0.2, 1) 0.2s both;
        }
        .animate-line-sep {
          animation: line-rise 0.9s cubic-bezier(0.2, 0.8, 0.2, 1) 0.6s both;
        }
        .animate-line-2 {
          animation: line-rise 0.9s cubic-bezier(0.2, 0.8, 0.2, 1) 0.85s both;
        }

        /* Generic rise — used by Slide 05 phones (delay set inline) */
        .animate-rise {
          animation: line-rise 0.8s cubic-bezier(0.2, 0.8, 0.2, 1) both;
        }
      `}</style>

      {/* Full-viewport shell — dark bg, glows, dot grid */}
      <div className="w-screen h-screen bg-[#020204] text-white font-sans overflow-hidden select-none relative flex items-center justify-center">
        ...
        <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
          ...
          <div
            className="absolute bottom-[-20%] right-[-10%] w-[60%] h-[60%] bg-[#4A48D4]/10 rounded-full blur-[150px] animate-glow mix-blend-screen"
        ...
        {/* Scaled slide stage — fixed design canvas, scaled to viewport */}
        <div
        ...
        >
          ...
          <div
            key={currentSlide}
            className="w-full h-full slide-enter flex items-center justify-center"
            onClick={goNext}
          >
